...
- [x] 新增全局任务队列入口，移动端底部任务面板，桌面/Web 居中任务面板。
- [x] 新增 `AppTaskRecoveryPolicy`，任务队列可展示中断恢复策略。
...
- [x] 形成 `AppTaskStatusSurface`，移动端使用底部任务面板，桌面/Web 使用居中或右侧任务面板。
- [x] 支持单任务覆盖和多任务队列两种 presentation。

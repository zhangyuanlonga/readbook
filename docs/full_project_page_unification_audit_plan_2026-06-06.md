...
- [ ] U5-BS-08 抽封面尺寸、占位和自适应策略到 `bookshelf_cover_layout_resolver.dart`，补尺寸测试。
- [ ] U5-BS-09 抽进度条 / 未读章节数展示到 `widgets/bookshelf_progress_indicator.dart`，网格和列表共用。
- [ ] U5-BS-10 抽书籍更多菜单 action 和 UI 到 `widgets/bookshelf_book_more_menu.dart`，移动端 bottom sheet、桌面 popover 入口保持原样。
...
- [ ] U5-ATL-10 抽批量导入队列、进度、汇总弹层到 `advanced_theme_batch_import_controller.dart`，补队列状态单测。
- [ ] U5-ATL-11 抽批量导出、批量分类、批量删除到 `advanced_theme_batch_action_controller.dart`。
- [ ] U5-ATL-12 抽删除确认和资源引用决策到 `advanced_theme_delete_decision_surface.dart`。
- [ ] U5-ATL-13 抽预览壁纸 image provider 缓存到 `advanced_theme_preview_image_cache.dart`，补缓存清理测试。
- [ ] U5-ATL-14 清理已迁移 helper，跑高级主题 list 目标 analyze / tests。
...
- [ ] BL-05-02 高级主题列表先执行 `U5-ATL-01` 到 `U5-ATL-06`，拆查询、卡片、状态、toolbar，再接入 route topbar，对应 `U3-FIX-03`、`U3-FIX-04`。
...
- [ ] BL-06-01 建立页面骨架接入清单：Shell 页面、独立路由页面、沉浸式阅读器分别标明 scaffold 策略。
- [ ] BL-06-02 统一操作弹层：手写 dialog、bottom sheet、右键菜单和更多菜单迁移到 adaptive surface。
- [ ] BL-06-03 统一加载、空态、错误、权限不足、平台能力缺失状态组件。
- [ ] BL-06-04 统一工具栏组件：搜索、排序、筛选、视图切换、导入 / 导出、更多操作进入 adaptive toolbar。
- [ ] BL-06-05 统一颜色、圆角、边距、阴影、字号、分割线 token，减少页面硬编码。
- [ ] BL-06-06 统一桌面 hover、focus、selected、disabled、loading、快捷键和右键菜单反馈。
- [ ] BL-06-07 统一移动端触控命中区域、Safe Area、文字缩放、底部弹层高度和小屏 overflow。

...
- 这 5 个 override 暂不直接删除，因为它们都绑定阅读器、本地内容或 PDF / 编码 / 翻页高风险能力。
...
- M2-10 移动端补构建已确认：Android 需要在根 Gradle 配置覆盖旧插件写死的 Kotlin language / api version，避免 `sentry_flutter 8.14.2` 的 Kotlin 1.6 配置被 Kotlin 2.x 工具链拒绝；iOS `--no-codesign` 当前阻塞在 `pdfium_flutter` 从 GitHub 下载 `PDFium.xcframework`，需要 GitHub 可达或预置 xcframework 后再补验。

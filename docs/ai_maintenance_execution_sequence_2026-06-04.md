...
M1 已完成，它来自旧第四里程碑的成熟库、生成工具和架构样板治理结论。后续任务默认继承 M1，不再重复执行旧 M1-M3。
...
当前默认从 M2 继续执行。不要因为用户提出某个功能点，就直接跳到页面写 UI。先判断它是否依赖手搓替换、业务链兼容、本地资源成熟化或长期门禁。
...
1. 分类：任务属于 M2 手搓替换、M3 业务兼容、M4 资源性能、M5 长期门禁中的哪一类。

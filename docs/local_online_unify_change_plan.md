...
## 目标
- [x] 书架入口与阅读入口统一，用户感知一致。
- [x] 本地导入与在线书源复用同一套阅读页、阅读进度与设置。
- [x] 保持现有书源规则引擎不变，降低改造风险。

## 非目标
- [x] 不改造书源规则引擎语法或兼容性。
- [x] 不在本阶段引入复杂的漫画/图像解析能力。
...
### 7) 数据与进度一致性
- [x] 本地阅读进度保持 scheme：
- [x] `detailUrl = local://book/{bookId}`
- [x] `chapterUrl = local://chapter/{chapterId}`
- [x] 本地 Provider 保证上述 scheme 与现有进度一致。
...
## 测试清单
- [x] 本地 TXT/EPUB 导入后可进入统一阅读页。
- [x] 本地阅读进度保存与恢复正常。
- [x] 在线书源加载目录与正文正常。
- [x] 换源、缓存按钮对本地书隐藏或禁用。
- [x] 本地书“重新索引”入口可用并可刷新目录。
- [x] 旧路由 `/local/*` 仍能打开并正确跳转到统一页面。
- [x] 异常路径：本地文件缺失、解析失败、空章节提示正确。

## 兼容性与迁移注意事项
- [x] 不破坏 `ReadingProgress` 字段要求。
- [x] 不改动规则引擎与书源格式。
- [x] 保持本地 `sourceId` 常量不变：`__local_book__`。
- [x] 本地 `detailUrl` 与 `chapterUrl` scheme 不变。

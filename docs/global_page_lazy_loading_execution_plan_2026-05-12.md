...
| Phase 6 | 验证与回归基线 | 建立启动、首屏、滚动、能力关闭验证 | 全局 | 已完成 |
...
- [x] 建立路由覆盖检查：所有 `routes.dart` 的 path 都出现在全局页面清单。
- [x] 建立首屏请求检查：冷启动进入 `/home`、`/bookshelf` 不出现书源/同步/图库/缓存统计请求。
- [x] 建立低端设备验证：Android 低内存设备书架滚动、阅读打开、我的页打开不明显卡顿。
- [x] 建立 Web 验证：默认能力关闭时 `/bookshelf`、`/mine`、`/search`、`/source`、`/sync` 不白屏。
- [x] 建立桌面验证：macOS/Windows/Linux 打开书架、详情、管理页不触发移动端专属能力异常。
- [x] 建立文档回填规则：完成阶段后勾选本文，并在页面清单补状态说明。

验收：

- [x] 有一套可重复执行的手工或自动化验证清单。
- [x] 每个阶段完成后有记录，不出现“代码改了但文档不知道”的情况。

执行记录：

- 新增 `tool/check_route_inventory.dart`，自动扫描 `lib/app/router.dart` 和 `lib/features/**/routes.dart` 中的 `path:`，校验它们都已出现在 `docs/global_page_route_inventory_2026-05-12.md`。
- `tool/run_architecture_green_suite.dart` 已接入路由清单覆盖检查，后续跑总回归会同步守住页面文档。
- 新增 `docs/global_page_lazy_loading_regression_checklist_2026-05-12.md`，把自动命令、首屏请求、Web、桌面、低端移动端和文档回填检查收敛成一份可重复清单。
- 本阶段完成后已同步回填阶段状态到本文和全局页面清单。

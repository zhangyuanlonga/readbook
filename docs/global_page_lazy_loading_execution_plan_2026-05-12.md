...
| Phase 4 | 我的与管理页懒初始化 | 低频管理功能进入页面后加载 | 外观、主题、图集、缓存、书签、反馈 | 待执行 |
| Phase 5 | Feature-gated 页面轻量化 | 能力关闭时不创建真实业务页面 | 书源、发现、搜索、同步 | 待执行 |
...
- [ ] `MinePage` 只读取入口配置和快照，不预加载管理页列表数据。
- [ ] 高级主题列表进入后加载；编辑器不由列表页提前创建。
- [ ] 启动图集、封面图库、底栏图标库进入后再扫描资源。
- [ ] 缓存管理进入后再统计缓存体积。
- [ ] 错误中心、反馈列表进入后再查询数据。
- [ ] 书签页进入后再查询书签；我的页不预查询。
- [ ] 标签管理、分类管理保持在数据标题下，但只在进入管理页后加载。

验收：

- [ ] 打开我的页不会触发图库扫描、缓存统计、反馈列表请求。
- [ ] 管理页返回我的页不会强制刷新所有入口数据。
- [ ] 低频页面独立失败时不影响核心书架、阅读、统计。
...
- [ ] 统一 `FeatureDisabledPage` 文案模型，按能力生成标题、说明和返回路径。
- [ ] `supportsSourceRuntime=false` 时，发现、搜索、书源页面不初始化真实 service。
- [ ] `supportsWebDavSync=false` 时，同步设置和同步历史不初始化同步 service。
- [ ] 主导航隐藏不可用发现 Tab，直接深链访问时显示轻量 disabled。
- [ ] 书源 debug、登录、编辑、导入这些非 GoRouter 页面在能力关闭时不可触发。

验收：

- [ ] 默认首版配置下，进入书源/搜索/发现/同步只显示占位，不报错、不白屏。
- [ ] 关闭能力时没有相关网络请求、WebView 初始化、脚本运行时初始化。
- [ ] 开启能力后，原页面仍可进入。

...
- [x] 补齐当前页可显示、缓存命中、缓存写入、首帧 ready 的细粒度 `Timeline` 标记。
- [x] 建立真机样本矩阵：低端 Android、中端 Android、iPhone、小屏模拟器，详见 `docs/reader_performance_validation_checklist_2026-05-13.md`。
- [x] 固定样本书：纯 TXT 大文件、普通 TXT、图文 EPUB、纯漫画、PDF，详见 `docs/reader_performance_validation_checklist_2026-05-13.md`。
- [x] 记录指标：打开到首屏、首翻页耗时、分页耗时、滚动帧率、翻页帧率、峰值内存、分页缓存命中率。
- [x] 构造压测章节：1 万行纯文本、50 张高清图混排、超长单段落。
- [x] 预留测试反馈入口：开发期可查看分页耗时、帧率、缓存命中和当前降级等级。
...
- [x] 新增 lazy page snapshot 模型，表达 `ready / loading / failed` 三态，页面消费快照而不是直接等整章列表。
- [x] 把 streaming pagination 改成逐页回调产出，当前/附近页可在完整分页结束前更新 UI。
- [x] 当前页优先：按恢复 ratio 映射目标段落，分页推进到目标段落时先发 current 事件。
- [x] 仅当前页、前页、后页进入高优先级计算队列；远页空闲时补齐，不阻塞首屏。
- [x] 翻到未计算页时展示骨架屏或上一页快照，计算完成后替换为真实页。当前实现先只暴露已计算页，避免进入空白未计算页。
- [x] 快速切章、改字号、改边距时继续沿用 `shouldAbort`，避免旧分页结果污染 UI。
- [x] 加分页耗时阈值日志，记录章节长度、段落数、页数、耗时。
...
- [x] 缓存 key 扩展：章节 ID、章节内容版本、字体、字号、行高、段距、页宽高、图片占位策略。
- [x] 图文分页完成后写入 block layout。
- [x] 命中缓存时直接恢复 `pagedBlockPages`。
- [x] 图片真实尺寸获取后，只在尺寸变化超阈值时触发重分页，避免小抖动导致全量失效。当前分页缓存以占位策略入 key，真实图片 decode 不反向触发全量分页。
...
- [x] 避免在 `PageView.builder` 的 `itemBuilder` 内做异步加载后全局 `setState`；分页页内容通过 page snapshot/listenable 精准更新。
- [x] 分页模式只保留水平分页手势，滚动模式只走纵向滚动和点击滚动。
- [x] 检查 `PageView` 物理参数和自定义 curl/cover/translate renderer 的重建范围。
- [x] 仿真翻页按设备等级降级：低端机默认平推或淡入淡出，中高端机再启用卷曲。
...
- [x] 对章节缓存、分页缓存、图片缓存记录条数和字节数。
- [x] 启动维护继续按预算清理，但日志要能看出清理原因。
- [x] 漫画分页只保留当前页、前后页的重型状态。
- [x] 滚动图文列表用 decode budget 控制图片尺寸，不使用全局 `imageCache.clear()` 粗暴清理。
- [x] 补缓存增长回归脚本或手工验收清单。
...
- [x] 梳理滚动模式段落 item 的高度策略：可固定高度的资源项使用固定约束，文本项避免额外测量。
- [x] 对超长单章建立段落窗口策略，只保留可见段落和缓冲区的重型状态。
- [x] 滚动回调不做全量 `setState`，进度展示改为独立 listenable/select 精准刷新。
- [x] 对图文滚动列表继续使用 decode budget，不做全局 `imageCache.clear()`。
- [x] 增加大 TXT 快速滚动 smoke 或手工压测清单。
...
- [x] 新增 reader device tier resolver，输入设备型号、系统版本、内存线索、电量和当前工作场景。
- [x] 把 tier 接入 `ReaderResourceBudget`。
- [x] 把 tier 接入翻页动画策略、图片 decode budget、邻章预加载和分页缓存数量。
- [x] 低电量和后台恢复场景自动收紧预算。

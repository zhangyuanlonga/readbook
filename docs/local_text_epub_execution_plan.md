...
- [x] 修复 TXT 导入后不可逆乱码问题
- [x] 固定 TXT 导入、索引、正文读取的单一编码事实来源
- [x] 提升 EPUB 目录粒度，补齐 `fragment` 级章节切分
- [x] 保持本地/在线共用同一套正文展示、分页、进度恢复链路
- [x] 补齐自动化测试与执行文档回填机制
...
- [x] 为 `LocalTextEncodingDetector` 补常见编码样本测试
- [x] 为 TXT 导入、索引、正文读取补 application / service 测试
- [x] 为 EPUB 目录粒度与章节读取补 application / parser 测试
- [x] 补本地阅读入口的 route / provider smoke test
- [x] 执行代表性回归验证：当前环境以自动化 smoke / integration-like 测试为主
- [x] 将已验证通过的项回填到本文件
- [x] 将关键结论同步更新到相关长期文档
...
- 状态：`已完成`
- 完成日期：`2026-04-27`
- 备注：已完成代表性自动化回归集，覆盖 TXT 编码样本、TXT 导入/索引/正文读取、EPUB 导入/目录粒度/正文解析、本地 provider 与 reader route smoke；长期文档已同步关键结论。当前环境未执行真机手点，但阶段 6 验收已用自动化 smoke / integration-like 覆盖替代。

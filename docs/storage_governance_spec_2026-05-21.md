...
- 本地图书“章节索引元数据”和“可重建正文缓存”必须分层
- `local_chapters` 只承载目录、偏移、`sourceRef`、轻量结构字段
- 可重建正文、图片列表、结构化正文缓存应进入独立正文缓存结构，不得继续与索引表混存
...
- 阅读器分页布局缓存属于可重建缓存，默认必须落入缓存目录口径
- 旧缓存目录可保留兼容读取窗口，但新写不得继续回流到业务托管目录
...
- EPUB 索引解析默认后台化
- TXT 继续按文件规模选择流式索引策略
- PDF 由于当前依赖 `MethodChannel` 文本提取插件，不强制 isolate 化，改走“轻索引 + 按页按需提取”

原因：

- EPUB zip 解压、目录提取、HTML 解析属于纯 Dart 重活，适合后台化
- PDF 当前插件链路不适合直接搬入 Dart isolate
- 先把 PDF 从“全书全文落库”改成“按需提取”，收益高于强推线程迁移

### 3.6 PDF 正文策略

定版结论：

- PDF 索引阶段只建立页目录、元数据和轻量定位信息
- PDF 正文在阅读时按页提取，首次提取后进入可重建正文缓存
- 不再允许索引阶段逐页全文写入数据库

...
- 接到桌面端、Web 端或移动端等单端任务时，先标记本次适用端和排除端，再改代码。
- 共用页面文件可以承载平台分流，但新增行为必须由断点、capability、adapter、facade 或明确平台语义保护。
- 不得为桌面端功能复用去重写移动端已稳定的 AppBar、底部弹层、更多菜单、排序弹层、搜索入口、选择模式和导航返回。
- 如确实需要调整共享状态或共享 provider，必须说明移动端是否会订阅、重建或改变行为；移动端不需要时不得让移动端监听桌面专用状态。
- 收尾时必须检查 `git diff`，确认排除端没有无意改动；如果共用文件发生变更，需在回复或任务记录中说明排除端为什么不受影响。

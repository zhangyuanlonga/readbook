...
- 官方主题预设免费可用且只能直接应用；自定义主题的编辑、复制、导入导出等能力走会员高级主题。
...
- 高级主题入口所有用户都能进入。
- 免费用户可以使用官方默认主题预设。
- 自定义主题创建、编辑、复制、导入导出、完整语义色和组件细调走会员。
...
免费用户可以直接使用官方预设。官方预设只允许应用，不提供复制后编辑；会员用户可从零创建或复制自定义高级主题。
...
- 官方主题可以和自定义主题在高级主题入口的同一列表中展示和启用。
- 官方主题不可直接编辑，也不可复制为自定义主题；自定义主题的创建、编辑、复制进入会员能力。
...
| P3 | 官方主题预设 registry | 内置 Lumina/Mono Blue/Ink Green/Selune Warm 官方预设并免费启用 | 100% | 完成：registry、统一主题列表、免费启用和官方预设只应用规则已接入 |
| P4 | 应用外观入口调整 | 重写“主题色/高级主题”的用户入口 | 100% | 完成：基础配色降级为 fallback，入口文案、Mine 摘要和路由 smoke 已更新 |
...
- [x] 高级主题列表页用同一列表展示官方主题和自定义主题，免费用户可见并可启用官方主题。
- [x] 官方主题只允许直接应用，不再提供复制后编辑入口。
- [x] 官方主题和自定义主题均为整卡点击应用；自定义主题不再显示“应用/停用”按钮，激活后展示右侧确认图标。
- [x] 新增主题来源、基础配色、官方主题启用和列表页免费可见测试。
- [x] P4 应用外观入口已迁移到“高级主题”，基础配色保留为 fallback 和旧数据迁移能力。
- [x] P5 全局按钮、输入框、chip、checkbox、switch、slider 与通用卡片 token 已收敛。
- [x] P6 官方主题免费使用、自定义主题会员 gate 和列表页回归测试已补齐。
- [ ] P0 组件样板截图路径未补齐。
...
目标: 内置官方主题预设，所有用户免费启用；自定义主题创建、编辑、复制和导入导出仍走会员能力。
...
- [x] 高级主题列表页将官方主题和自定义主题合并为同一个列表。
- [x] 免费用户可启用官方主题。
- [x] 非会员点击“创建/编辑/复制/导入/导出”自定义主题时走现有会员 gate。
- [x] 为官方主题启用、未知 preset fallback、免费启用和会员编辑 gate 补测试。

当前完成度: 100%。官方主题 registry、免费启用、整卡点击切换和“官方预设不可复制编辑”规则已完成；自定义主题会员 gate 由 P6 继续回归。
...
- [x] 官方主题不可直接编辑。
- [x] 官方主题不能复制编辑；创建、编辑、复制自定义主题和导入导出仍需要会员。
...
目标: 让用户能理解“明暗模式、官方主题预设、高级主题”的关系；基础配色只作为未启用高级主题时的 fallback，不常驻展示。
...
- [x] 将“颜色/主题色”入口从应用外观默认界面移除，避免默认官方主题下误导用户。
- [x] 基础配色保留为 fallback 和旧 seed 迁移能力，不作为默认外观入口。
- [x] 当未来提供“使用基础配色”入口时，再展示基础配色当前生效状态。
- [x] 新增或调整“高级主题”入口，展示当前启用来源。
- [x] 应用外观页移除组件样板入口。
- [x] 明暗模式继续保留在应用外观，并说明它切换当前主题来源的 light/dark 配置。
- [x] Mine 首页摘要显示明暗模式、主题来源、基础配色或高级主题名称。
- [x] 为入口文案和路由 smoke 补测试。

当前完成度: 100%。应用外观入口、Mine 摘要和高级主题入口已完成；基础配色已降级为 fallback。

验收:

- [x] 用户不会在默认官方主题下看到一个当前不生效的基础配色入口。
- [x] 页面不暗示免费用户只能换一个 seed 颜色。
- [x] 启用官方主题后，应用外观页不会暗示基础配色能立即改变当前外观。
- [x] 高级主题入口对免费用户可见，官方主题可直接应用。
...
- [x] 复核创建、编辑、复制自定义主题、导入、导出、完整语义色编辑、组件样式细调仍触发会员 gate。
...
- 主题列表
  ...
  - Ink Green: 官方，点击应用
  - Selune Warm: 官方，点击应用
  - 自定义主题: 点击应用，长按进入编辑页，生效时右侧显示确认标识
- 创建主题: 会员
- 复制自定义主题: 会员
- 导入/导出: 会员
```
...
复制自定义主题后编辑 = 会员高级主题能力；官方主题只能应用，不能复制编辑。
...
- [ ] 书架、搜索、阅读记录、设置页保持白底优先和轻层级。
- [x] 应用外观页不再常驻展示基础配色入口，避免默认官方主题下误导用户。

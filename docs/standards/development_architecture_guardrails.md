...
对规则清晰、生态成熟、维护活跃的能力，优先采用 Flutter 原生能力或成熟第三方库，不自行手搓底层实现。项目代码只负责封装统一主题、跨端行为、无障碍、错误处理和性能边界。

优先使用成熟方案的典型场景：

- 图片预览、缩放、缓存、解码约束。
- 骨架屏、基础动效、状态切换动画。
- 滑动操作、拖拽排序、上下文菜单、键盘快捷键。
- 下拉刷新、搜索高亮、Toast/Snackbar、触觉反馈。
- PDF、EPUB、编码检测、网络、缓存、数据库等已有成熟生态的基础能力。

允许自研的条件：

- Flutter 原生能力和成熟库无法覆盖项目的跨端、性能、许可或业务约束。
- 已评估维护活跃度、平台覆盖、license、包体影响和测试成本。
- 自研范围只补缺口，不重写成熟库已经稳定解决的核心能力。
- 自研实现必须有明确退出条件：后续如出现合适成熟库，可以迁移替换。

封装边界：

- 第三方库 API 不直接散落到业务页面；必须先通过 `app/`、`core/` 或 feature-local 组件/service 封装。
- 封装层负责接入 `ThemeData`、Token、Capability、Platform adapter、错误降级和测试。
- 新增依赖必须在 PR 中说明“为什么不用已有 Flutter 原生能力或当前依赖”。
...
- 冷启动进入 `/bookshelf`，不应触发服务器书源网关请求、同步、图库扫描、缓存统计。
...
16. 新增模型 / DTO / 状态对象是否仍在手写 `copyWith` / `toJson` / `fromJson` / equality，而没有使用 `freezed` / `json_serializable` 或登记技术债

只要第 1、2、4、6、10、11、13、15、16 任一项回答为“有”，默认不通过。
